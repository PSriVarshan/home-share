...
public interface BookingsRepo extends JpaRepository<BookingsModel, Integer> {
        ...
        @Query("SELECT b FROM BookingsModel b WHERE b.status = com.example.home_share.Model.Enums.BookingStatus.BOOKED")
        List<BookingsModel> currentBookingsYetToBeCompleted();

        @Transactional
        @Modifying
        @Query("UPDATE BookingsModel b SET b.status = com.example.home_share.Model.Enums.BookingStatus.CANCELLED WHERE b.bookingId=:bid")
        void cancel(
